    ...
    const mutation = useMutation({
        mutationFn: async () => {
            const channel = client.channel("messaging", {
                members: [loggedUser.id, ...selectedUsers.map((u) => u.id)],
                name:
...
function UserResult({user, selected, onClick}: UserResultProps){
    ...
    return <button className='flex w-full items-center justify-between px-4 py-2.5 transition-colors hover:bg-muted/50'
            onClick={onClick}
        >
